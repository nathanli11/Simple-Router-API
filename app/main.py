...
async def do_deposit(req: DepositRequest, username: str = Depends(_get_current_user)) -> dict:
    ...
    valid_assets = {split_symbol(sym)[0] for sym in SETTINGS.symbols} | {split_symbol(sym)[1] for sym in SETTINGS.symbols}
    if req.asset not in valid_assets:
        raise HTTPException(status_code=400, detail=f"invalid asset, must be one of {sorted(valid_assets)}")
