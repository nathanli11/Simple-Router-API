...
class TestWSServer(unittest.TestCase):
    ...
    def test_broadcast_best_touch_filters_by_symbol(self) -> None:
        async def run() -> None:
            ...
            await hub.broadcast_best_touch(
                "BTCUSDT", 100.0, 101.0, "binance", "okx",
                source_exchange="binance", src_bid=100.0, src_ask=102.0,
            )
    ...
    def test_update_ewma_initializes_then_smooths(self) -> None:
        async def run() -> None:
            ...
            await hub.add(conn)
            # Le filtre est strict: un subscriber exchange="all" ne recoit que les
            # appels avec exchange="all" (comme dans handle_trade).
            await hub.update_ewma_on_trade("BTCUSDT", "all", 100.0, 1000.0)
            await hub.update_ewma_on_trade("BTCUSDT", "all", 110.0, 1010.0)
